...
# dummy code to trigger jit compilation
env, obs = env.reset(PRNG_key=PRNG_key)
PRNG_key, key_env = jax.random.split(PRNG_key)

init_state = (env, obs, key_env, jnp.asarray(0.0, dtype=jnp.float32))
...
def step_fn(dynamic_carry, _):
    ...
    env, (next_obs, rews, dones, info) = env.step(PRNG_key=key_step_i, actions=actions)
    rewards = jnp.stack(rews, axis=1, dtype=jnp.float32)
...
@jax.jit
@chex.assert_max_traces(2)
def fn(dynamic_init_state):
    print("tracing")
    final_carry, _ = jax.lax.scan(step_fn, dynamic_init_state, None, length=n_steps)
    return final_carry


final_carry = fn(dynamic_init_state)

env, obs, _, total_reward = eqx.combine(static_state, final_carry)
...
step = 0
env, obs = env.reset(PRNG_key=key_env)
total_reward = 0.0
if render:

...
class VelocityController:
    ...
    def reset(self, index: int | None = None):
        if index is None:
            self.accum_errs = jnp.zeros(
                (self.world.batch_dim, self.world.dim_p),
                device=self.world.device,
            )
            self.prev_err = jnp.zeros(
                (self.world.batch_dim, self.world.dim_p),
                device=self.world.device,
            )
        else:
            self.accum_errs = JaxUtils.where_from_index(index, 0.0, self.accum_errs)
            self.prev_err = JaxUtils.where_from_index(index, 0.0, self.prev_err)

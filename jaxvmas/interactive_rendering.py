...
class InteractiveEnv:
    ...
    def __init__(
        ...
        self.n_agents = len(self.env.scenario.agents)
        self.agents = self.env.scenario.agents
        self.continuous = self.env.continuous_actions
        self.reset = False
        self.keys = jnp.zeros(6)  # up, down, left, right, rot+, rot-
        self.keys2 = jnp.zeros(6)  # up, down, left, right, rot+, rot-
        ...
        self.text_lines = []
        self.font_size = 15
        self.env_state = self.env.render(self.env_state)
    ...
    def _cycle(self):
        ...
        while True:
            if self.reset:
                if self.save_render:
                    save_video(
                        self.render_name,
                        self.frame_list,
                        fps=1 / self.env.world.dt,
                    )
                self.env_state = self.env.reset(self.env_state)[0]
                self.reset = False
                total_rew = [0] * self.n_agents
            ...
            self.env_state, (obs, rew, done, info) = self.env.step(
                self.env_state, action_list
            )
            ...
            if self.display_info and self.n_agents > 0:
                ...
                message = f"Rew: {round(float(rew[self.current_agent_index]),3)}"
                ...
                message = f"Selected: {self.env.scenario.agents[self.current_agent_index].name}"
            ...
            self.env_state = frame[0] if isinstance(frame, tuple) else frame
            if self.save_render and isinstance(frame, tuple):
                self.frame_list.append(frame[1])
    ...
    def _key_press(self, k, mod):
        ...
        agent_range = self.agents[self.current_agent_index].action.u_range
        try:
            if k == key.LEFT:
            ...
            if self.control_two_agents:
                agent2_range = self.agents[self.current_agent_index2].action.u_range
    ...
    def set_u(self):
        if self.continuous:
            self.u = [
                float(self.keys[1] - self.keys[0]),
                float(self.keys[3] - self.keys[2]),
                float(self.keys[4] - self.keys[5]),
            ]
            self.u2 = [
                float(self.keys2[1] - self.keys2[0]),
                float(self.keys2[3] - self.keys2[2]),
                float(self.keys2[4] - self.keys2[5]),
            ]
        else:
            if jnp.sum(self.keys[:4]) >= 1:
                self.u[0] = int(jnp.argmax(self.keys[:4])) + 1
            else:
                self.u[0] = 0
            if jnp.sum(self.keys[4:]) >= 1:
                self.u[1] = int(jnp.argmax(self.keys[4:])) + 1
            else:
                self.u[1] = 0

            if jnp.sum(self.keys2[:4]) >= 1:
                self.u2[0] = int(jnp.argmax(self.keys2[:4])) + 1
            else:
                self.u2[0] = 0
            if jnp.sum(self.keys2[4:]) >= 1:
                self.u2[1] = int(jnp.argmax(self.keys2[4:])) + 1
            else:
                self.u2[1] = 0

    @staticmethod
    def format_obs(obs: PyTree) -> Any:
        if isinstance(obs, Array):
            return jnp.around(obs, decimals=2).tolist()
    ...
    **kwargs,
    ...
    Args:
        scenario (Union[str, BaseScenario]): Scenario to load.
            Can be the name of a file in `jaxvmas.scenarios` folder or a :class:`~jaxvmas.simulator.scenario.BaseScenario` class
    ...
    Examples:
        >>> from jaxvmas import render_interactively
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="waterfall",
        help="Scenario to load. Can be the name of a file in `jaxvmas.scenarios` folder or a :class:`~jaxvmas.simulator.scenario.BaseScenario` class",
    )
    parser.add_argument(
        "--control_two_agents",
